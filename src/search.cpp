...
moves_loop: // When in check, search starts from here
    ...
    while ((move = mp.next_move(skipQuiets)) != MOVE_NONE)
    {
      ...
      if (type_of(capturedPiece) == QUEEN && depth < 6 * ONE_PLY)
      {
          ...
          {
              Square qsq = pos.square<QUEEN>(us);
              bool attacked =   attacks_bb<ROOK  >(qsq, pos.pieces()) & pos.pieces(~us, ROOK  )
                             || attacks_bb<BISHOP>(qsq, pos.pieces()) & pos.pieces(~us, BISHOP)
                             || pos.attacks_from<KNIGHT>(qsq)         & pos.pieces(~us, KNIGHT)
                             || pos.attacks_from<PAWN  >(qsq, us)     & pos.pieces(~us, PAWN  );
              if (attacked)
                  extension = ONE_PLY;
          }
      }
